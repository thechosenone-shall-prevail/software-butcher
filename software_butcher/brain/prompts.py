...
- NEVER probe CTF/lab filesystem paths (/home/user/user.txt, /root/root.txt, flag.txt) on real web assessments unless those paths appear organically in HTML, redirects, or tool output.
...
3. If stack_landing indicates XAMPP/default hosting, run at most 2–3 hostname-derived semantic probes — never spray every substring (/book, /booking, /hall).

4. Prefer http_surface_map (local) over web_behavior_analysis (HexStrike) for phpMyAdmin, phpinfo, and MySQL/resource-exhaustion conclusions.
...
8. Reject CTF patterns: do not choose capabilities targeting /home/, user.txt, root.txt, or flag files unless organically discovered.
...
8. Never prioritize CTF filesystem paths (/home/user/user.txt, flag.txt) on production web portal assessments.
