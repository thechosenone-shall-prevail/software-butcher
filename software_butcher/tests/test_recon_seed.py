...
def test_mark_host_recon_advances_checklist(tmp_path):
    ...
    mark_host_recon(store.recon_checklist, store.base_target, "http_surface_map")
    assert store.recon_checklist.is_complete("hallbooking.srmrmp.edu.in")


def test_ensure_host_recon_requeues_surface_map(tmp_path):
    store = FindingStore(tmp_path / "state.json")
    store.set_base_target("http://hallbooking.srmrmp.edu.in")
    assert ensure_host_recon_hypothesis(store) is True
    hyp = next_recon_hypothesis(store)
    assert hyp is not None
    assert hyp.metadata["intent"] == "http_surface_map"
